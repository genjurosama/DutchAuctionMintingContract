...
*/
    ...
    address public piraDexAdress;

    //Mapping of pirate to timestamp
    mapping(uint256 => uint256) internal tokenIdToTimeStamp;

    //Mapping of pirate to staker
    mapping(uint256 => address) internal tokenIdToStaker;

    //Mapping of staker to pirates
    mapping(address => uint256[]) internal stakerToTokenIds;

    constructor() ERC20("Gold", "GOLD") {}

    function setPiraDexAdress(address _piraDexAdress) public onlyOwner {
        piraDexAdress = _piraDexAdress;
        return;
    }
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(piraDexAdress).ownerOf(tokenIds[i]) == msg.sender &&
                    tokenIdToStaker[tokenIds[i]] == nullAddress,
                "Token must be stakable by you!"
            );

            IERC721(piraDexAdress).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
